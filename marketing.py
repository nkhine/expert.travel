    ...
    def take_exam_result(self, context):
        ...
        for key in question_keys:
            ...
            if question is not None:
                ...
                #question_ns['is_correct'] = 'REMOVE' 
                #question_ns['answer'] = 'REMOVE ANSWER' 
                # Options
                options = []
        ...
        #namespace['user_attempts'] = 'REMOVE'
        #namespace['time_taken'] = '20'
        #namespace['score'] = str(self.get_score(user.name))
        #namespace['questions_nums'] = 'REMOVE' 
        ...
        handler = self.get_handler('/ui/abakuc/marketing/response.xml')
        return stl(handler, namespace)
...
#class Survey(Text):
#
#    __slots__ = ['uri', 'timestamp', 'parent',
#                 'name', 'real_handler', 'questions']
#
#
#    def new(self):
#        self.questions = {}
...
#    def get_survey(self):
#        return self.get_handler('.survey')
#    definition = property(get_survey, None, None, '')
#
#
#    def get_response(self):
#        return self.get_handler('.response')
#    response = property(get_response, None, None, '')
#
#
#    #########################################################################
#    # Index and search
...
#    def get_score(self, username):
#        score = 0.0
#        last_attempt = self.response.get_last_attempt(username)
#        if last_attempt:
#            score = last_attempt.get_score()
#
#        return score
...
#
#
#    def get_response(self, username=None):
#        """
#        Return a four-elements tuple:
#        - has the user already filled the Marketing Form (bool)
#        - date when this was made 
#        """
#        if username is None:
#            username = get_context().user.name
#
#        #n_attempts = self.response.get_n_attempts(username)
#        #if n_attempts == 0:
#        #    return False, ''
#
#        #submission_date = self.response.get_submission_date(username)
#
#        #date = Date.encode(submission_date.date)
#        return u'response'
#
#
#    def get_module(self):
...
#    def remove(self, context):
#        codes = context.get_form_values('codes')
#        self.survey.set_changed()
#        for code in codes:
#            del self.survey.questions[code]
#
#        return context.come_back(u'Question(s) removed.')
#
#
...
#
#        # Get the survey
#        survey = self.definition
#        survey.set_changed()
#        # Modify the question
#        question = survey.questions.pop(old_code)
#        question.code = new_code
#        question.title = title
#        question.options = options
#        survey.questions[new_code] = question
#
#        goto = ';edit_question_form?code=%s' % new_code
#        return context.come_back(u'Question changed.', goto=goto)
#
...
#        survey = self.definition
#        survey.set_changed()
#        survey.questions[code] = question
#
#        return context.come_back(u'Question added.', goto=';edit')
#
#    #########################################################################
#    # Fulfill form 
#    fill_form__access__ = 'is_allowed_to_edit'
#    fill_form__label__ = u'Fulfill marketing form'
...
#        return data
#
#    fill__access__ = 'is_allowed_to_edit'
...
#        exam_questions = self.response.questions
#        for key in questions:
#            if key in exam_questions:
#                value = context.get_form_values(key)
#                value = [ int(x) + 1 for x in value ]
#                attempt.questions[key] = value
#
#        self.response.set_changed()
#        attempts = self.response.attempt.setdefault(username, [])
#        attempts.append(attempt)
#        # Redirect to the results page
#        return context.come_back(u'XXX', ';response')
#
#
#    response__access__ = 'is_allowed_to_edit' 
#    def response(self, context):
#        user = context.user
#        # Metadata
#        fill = self.get_response(user.name)[0]
#
#        #handler = self.get_handler('/ui/abakuc/marketing/response.xml')
#        #return context.come_back(u'XXX', '../;end')
#        return u'XXX'
#
#register_object_class(Marketing)
